...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> <a href="<?php echo U('Index/welcome');?>">首页</a><span class="c-gray en">&gt;</span> 新闻管理 <span class="c-gray en">&gt;</span> 新闻列表 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="page-container">
	<button onclick="removeIframe()" class="btn btn-primary radius">关闭选项卡</button>
	 <div >
		<form method="get" action="/index.php/Admin/News/passnews_list.html">
	...
	<div class="mt-20">
		...
		<table class="table table-border table-bordered table-bg table-hover  table-responsive"  id="example">
			...
			<?php if(is_array($newlist)): foreach($newlist as $key=>$vo): ?><tr class="text-c va-m check-tr">
				    ...
				    <td> <input type="text"  class="input-text" onchange="changeOrder(this,'<?php echo ($vo["id"]); ?>','<?php echo ($vo["cate_id"]); ?>')"  value="<?php echo ($vo["sort_id"]); ?>"></td>
					<td><?php echo ($vo["id"]); ?></td>
				    <td>
						<input type="text" hidden class="input-text"  id="ordervalue" value="<?php echo ($vo["sort_id"]); ?>">
						<a onclick="decOrder(this,'<?php echo ($vo["id"]); ?>','<?php echo ($vo["cate_id"]); ?>')"><i class="Hui-iconfont" style="color: red;">&#xe674;</i></a>&nbsp;&nbsp;
						<a onclick="incOrder(this,'<?php echo ($vo["id"]); ?>','<?php echo ($vo["cate_id"]); ?>')"><i class="Hui-iconfont" style="color: green">&#xe679;</i></a>
					</td>
				    ...
				    <td  style="cursor:pointer;text-align: center" onclick="is_top(this,'<?php echo ($vo["id"]); ?>','<?php echo ($vo["cate_id"]); ?>')"  class="is_top  radius"><strong><?php echo $vo['is_top']==1?'置顶':'不置顶'; ?></strong></td>
...
// 为启用的td加一个事件，是否制定
function is_top(obj,id,cate_id) {
		$.ajax({
			type : "GET",
			...
			url : "/index.php/Admin/News/ajaxUpdateIstop",
...
//排序
function changeOrder(obj,id,cate_id){
	var sort_id = $(obj).val();
//	alert(cate_id);
	$.post("/index.php/Admin/News/changeOrder",{'id':id,'sort_id':sort_id,'cate_id':cate_id},function(data){
		if(data){
...
//排序值名次增加，该操作sort_id减一，
function incOrder(obj,id,cate_id){
	var sort_id = $("#ordervalue").val();
	$.post("/index.php/Admin/News/incOrder",{'id':id,'sort_id':sort_id,'cate_id':cate_id},function(data){
		if(data==1){
			...
			if(rowIdx==1){
				layer.msg('已经是在排序最前面了！',{icon:5,time:2000});
				return;
			}
				//前一个tr的索引值
		 ...
		 location.replace(location.href);
				...
				layer.msg('排序增加成功!',{icon:6,time:2000});
		    location.replace(location.href);

		}
		  else {
			layer.msg('已经是排序最大值了！',{icon:5,time:2000});
		 }
	});

}
//排序值名次减小，该操作sort_id加一，
function decOrder(obj,id,cate_id){
	var sort_id = $("#ordervalue").val();
	$.post("/index.php/Admin/News/decOrder",{'id':id,'sort_id':sort_id,'cate_id':cate_id},function(data){
//		alert (data);
		if(data==1){
//增加索引，判断排序值
			var tr=obj.parentNode.parentNode;
			...
			layer.msg('排序减小成功!',{icon:6,time:2000});
			location.replace(location.href);

		}else{
			layer.msg('已经是排序最小值了',{icon:5,time:2000});
		}
	});
}
/*新闻-批量删除*/
